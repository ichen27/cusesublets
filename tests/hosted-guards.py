...
class HostedGuards(unittest.TestCase):
    ...
    def test_private_endpoints_need_signed_identity(self):
        for endpoint in ["/api/profile","/api/admin/identities","/api/identity-documents/not-a-document","/api/conversations","/api/chat-documents/not-a-document","/api/mine","/api/messages","/api/offers","/api/bookings","/api/admin/overview"]:
    ...
    def test_mutations_require_identity_and_exact_origin(self):
        for endpoint in ["/api/profile","/api/conversations","/api/messages","/api/offers","/api/listings"]:
