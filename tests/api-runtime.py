...
class API(unittest.TestCase):
 def test_full_trust_flow(self):
  ...
  self.assertEqual(host.call('/api/listings',{**payload,'lat':44})[0],400)
  self.assertEqual(host.call('/api/listings',{**payload,'lng':-75})[0],400)
  payload.update(lat=43.042123,lng=-76.125987)
  status,result=host.call('/api/listings',payload);self.assertEqual(status,201,result);key=result['listing']['id'];self.assertEqual(result['listing']['lat'],43.042);self.assertEqual(result['listing']['lng'],-76.126)
  ...
  for kind,other in [('lease','permissionStatus'),('permission','leaseStatus')]:
   ...
   self.assertEqual(admin.call('/api/admin/listings/'+key+'/review',review)[0],200)
   self.assertNotIn('Required reviews incomplete',current_booking()['payoutBlockers'])
  # Reports and account moderation are audited, with existing dispute access retained.
  status,result=renter.call('/api/listings/'+key+'/report',dict(reason='Runtime sample: listing needs a support review'))
  self.assertEqual(status,201,result);rid=result['report']['id']
  self.assertEqual(host.call('/api/listings/'+key+'/report',dict(reason='Owner cannot report their own listing'))[0],400)
  self.assertEqual(renter.call('/api/listings/'+key+'/report',dict(reason='Duplicate open report must be rejected'))[0],409)
  self.assertTrue(any(x['id']==rid for x in admin.call('/api/admin')[1]['reports']))
  self.assertEqual(renter.call('/api/admin/users/demo-host/status',dict(suspended=True,reason='Not authorized'))[0],403)
  self.assertEqual(admin.call('/api/admin/users/demo-admin/status',dict(suspended=True,reason='Self suspension prohibited'))[0],403)
  self.assertEqual(admin.call('/api/admin/users/demo-host/status',dict(suspended=True,reason=''))[0],400)
  try:
   self.assertEqual(admin.call('/api/admin/users/demo-host/status',dict(suspended=True,reason='Runtime moderation safety check'))[0],200)
   self.assertTrue(host.call('/api/session')[1]['user']['suspended'])
   self.assertEqual(guest.call('/api/listings/'+key)[0],404)
   self.assertNotIn(key,[l['id'] for l in guest.call('/api/listings')[1]['listings']])
   self.assertEqual(host.call('/api/listings/'+key)[0],200)
   self.assertEqual(guest.raw(media)[0],404)
   self.assertEqual(host.call('/api/listings',payload)[0],403)
   self.assertEqual(host.call('/api/offers/'+oid2+'/accept',{})[0],403)
   self.assertEqual(renter.call(path,dict(action='pay'))[0],409)
   self.assertIn('Account suspension requires review',current_booking()['payoutBlockers']);self.assertFalse(current_booking()['payoutEligible'])
  finally: self.assertEqual(admin.call('/api/admin/users/demo-host/status',dict(suspended=False,reason='Restore account after runtime verification'))[0],200)
  try:
   self.assertEqual(admin.call('/api/admin/users/demo-renter/status',dict(suspended=True,reason='Runtime renter safety check'))[0],200)
   self.assertEqual(renter.call('/api/bookings')[0],200)
   self.assertEqual(renter.call('/api/offers',offer)[0],403)
   self.assertEqual(renter.call(path,dict(action='sign'))[0],403)
   self.assertEqual(renter.call(path,dict(action='pay'))[0],403)
   self.assertEqual(renter.call(path,dict(action='dispute',reason='Suspended renters retain support access'))[0],200)
  finally: self.assertEqual(admin.call('/api/admin/users/demo-renter/status',dict(suspended=False,reason='Restore account after runtime verification'))[0],200)
  self.assertEqual(admin.call('/api/admin/reports/'+rid+'/resolve',dict(reason=''))[0],400)
  self.assertEqual(admin.call('/api/admin/reports/'+rid+'/resolve',dict(reason='Runtime report reviewed and resolved'))[0],200)
  self.assertEqual(admin.call('/api/admin/bookings/'+bid+'/resolve',dict(reason='Runtime safety dispute resolved'))[0],200)
  self.assertFalse(host.call('/api/session')[1]['user']['suspended'])
  self.assertEqual(guest.call('/api/listings/'+key)[0],200)
  audit_actions=[x['action'] for x in admin.call('/api/admin')[1]['audit']]
  for action in ['listing.report','user.suspend','user.restore','report.resolve']:self.assertIn(action,audit_actions)
